...
def load_pybullet(filename, fixed_base=False, scale=1., **kwargs):
    ...
    with LockRenderer():
        if filename.endswith('.urdf'):
            ...
            print("loading urdf,current urdf directory is ",filename)
...
def joint_from_name(body, name):
    #print("##############body is",get_joints(body))
    for joint in get_joints(body):
        #print("getting joint name", get_joint_name(body, joint))
        #print("name is ",name)
...
def set_joint_position(body, joint, value):
    ...
    # TODO: pybullet will automatically compute FK so we should replace it with our own FK
...
def set_joint_positions(body, joints, values):
    #print("joints is ", joints)
    #print('value is values', values)
